...
run_sim_alt <- function(rep_id, m, T.chan, gamma, delta = 1, s.star = 200, alpha = 0.05, dgp_type = "BB") {
  ...
  # Inject smooth linear drift at t* = m + s.star
  t_star <- m + s.star
  change_idx <- t_star:total_n
  num_change <- length(change_idx)
  
  # Create linear weights from 0 to delta
  if (delta == 0) {
    drift_weights <- rep(0, num_change)
  } else {
    drift_weights <- seq(0, delta, length.out = num_change)
  }
  
  # Add drift to each post-change curve
  for (i in seq_along(change_idx)) {
    fd_data$coefs[, change_idx[i]] <- fd_data$coefs[, change_idx[i]] + drift_weights[i]
  }
...
# delta_vals <- seq(0.05, 0.13, by = 0.015)
 delta_vals <- 0
...
output_dir <- "output_alt_smooth_change"
