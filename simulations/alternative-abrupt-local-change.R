...
run_sim_alt <- function(rep_id, m, T.chan, gamma, delta = 1, s.star = 200, alpha = 0.05, dgp_type = "BB", basis_k = 5) {
  total_n <- m + m * T.chan
  ...
  # Inject abrupt localized change after t* = m + s.star
  t_star <- m + s.star
  change_idx <- t_star:total_n
  
  # Create the delta * b_k(s) localized change as an fd object
  basis <- create.bspline.basis(rangeval = c(0, 1), nbasis = 21)
  bk_coefs <- matrix(0, nrow = 21, ncol = 1)  # matrix, not vector!
  bk_coefs[basis_k, 1] <- delta
  mu_fd <- fd(coef = bk_coefs, basisobj = basis)
  
  # Add this change in coefficient space (21 × 1) to each observation
  for (i in change_idx) {
...
# Parameters
 ...
 delta_vals <- seq(from = 0.1, to = 1.7, by = 0.3)
...
output_dir <- "output_alt_abrupt_local_change"
