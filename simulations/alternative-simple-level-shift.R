...
run_sim_alt <- function(rep_id, m, T.chan, gamma, delta = 1, s.star = 200, alpha = 0.05, dgp_type = "BB") {
  ...
  # Inject global shift at m + s.star
  change_idx <- (m + s.star):total_n
  fd_data$coefs[, change_idx] <- fd_data$coefs[, change_idx] + delta
...
gamma_vals <- c(0, 0.15)
 ...
 delta_vals <- c(0,  seq(0.05, 0.13, by = 0.015))
...
output_dir <- "output_alt_level_shift"
